...
export type TransactionType = 'Purchase' | 'Dispense' | 'Adjustment' | 'Expired' | 'Damaged' | 'Returned' | 'Transfer'

export async function recordStockTransaction(
  supabase: SupabaseClient,
  payload: Omit<StockTransactionRow, 'id' | 'created_at'> & { transaction_type: TransactionType }
): Promise<void> {
